...
use qq_message::{
    MessageDecoder, MessageDisposition, decode_friend_history_response,
    decode_group_history_response, decode_group_reaction, decode_rich_text,
...
fuzz_target!(|data: &[u8]| {
    ...
    if selector & 32 != 0 {
        let _result = decode_friend_history_response(body, "u_peer", 1, 2, 1, 1);
        return;
    }
    if selector & 16 != 0 {
